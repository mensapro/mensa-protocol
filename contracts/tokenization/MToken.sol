...
import "../mensa/Mensa.sol";
import "../mensa/MensaDataProvider.sol";
import "../mensa/MensaCore.sol";
import "../libraries/WadRayMath.sol";

/**
 * @title Mensa ERC20 MToken
 *
...
contract MToken is ERC20, ERC20Detailed {
    ...
    **/
    ...
    MensaAddressesProvider private addressesProvider;
    MensaCore private core;
    Mensa private mensa;
    MensaDataProvider private dataProvider;

    modifier onlyMensa {
        require(
            msg.sender == address(mensa),
            "The caller of this function must be a mensa"
        );
        _;
    }
    ...
    **/
    modifier onlyMensaStaff {
        require(address(mensa) == msg.sender ||  
        addressesProvider.getMensaLiquidationManager() == msg.sender, "The caller must be a mensa staff contract");
        _;
    }
    ...
    constructor(
        MensaAddressesProvider _addressesProvider,
        ...
        mensa = Mensa(addressesProvider.getMensa());
    ...
    **/
    ...
    /**
     * @dev mints token in the event of users depositing the underlying asset into the mensa 
     * only mensa can call this function
     * @param _account the address receiving the minted tokens
     * @param _amount the amount of tokens to mint
     */
    function mintOnDeposit(address _account, uint256 _amount) external onlyMensa {
     ...
     * Transfer of the liquidated asset is executed by the mensa contract.
     * only mensa can call this function
     * @param _account the address from which burn the mTokens
     * @param _value the amount to burn
     **/
    function burnOnLiquidation(address _account, uint256 _value) external onlyMensaStaff {
    ...
    /**
     * @dev transfers tokens in the event of a borrow being liquidated, in case the liquidators reclaims the mToken
     *      only mensa can call this function
     * @param _from the address from which transfer the mTokens
     * @param _to the destination address
     * @param _value the amount to transfer
     **/
    function transferOnLiquidation(address _from, address _to, uint256 _value) external onlyMensaStaff {

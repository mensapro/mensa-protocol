...
import "../configuration/MensaParametersProvider.sol";
...
import "./MensaLiquidationManager.sol";
import "../libraries/EthAddressLib.sol";

/**
* @title Mensa contract
* @notice Implements the actions of the Mensa, and exposes accessory methods to fetch the users and reserve data
* @author Mensa
 **/

contract Mensa is VersionedInitializable {
    using SafeMath for uint256;
    using WadRayMath for uint256;
    using Address for address;

    MensaAddressesProvider public addressesProvider;
    MensaCore public core;
    MensaDataProvider public dataProvider;
    MensaParametersProvider public parametersProvider;
    IFeeProvider feeProvider;
    MensaLiquidationManager public liquidationManager;
    IMensaMinter minter;
    ...
    * @param _receiveMToken true if the liquidator wants to receive MTokens, false otherwise
    * @param _timestamp the timestamp of the action
    **/
    ...
    **/
    ...
    /**
    * @dev this function is invoked by the proxy contract when the Mensa contract is added to the
    * AddressesProvider.
    * @param _addressesProvider the address of the MensaAddressesProvider registry
    **/
    function initialize(MensaAddressesProvider _addressesProvider) public initializer {
         _notEntered = true;
        addressesProvider = _addressesProvider;
        core = MensaCore(addressesProvider.getMensaCore());
        dataProvider = MensaDataProvider(addressesProvider.getMensaDataProvider());
        parametersProvider = MensaParametersProvider(
            addressesProvider.getMensaParametersProvider()
        );
        feeProvider = IFeeProvider(addressesProvider.getFeeProvider());
        minter = IMensaMinter(addressesProvider.getMensaMinter());
    }

    /**
    * @dev deposits The underlying asset into the reserve. A corresponding amount of the overlying asset (MTokens)
    * is minted.
    * @param _reserve the address of the reserve
    * @param _amount the amount to be deposited
    ...
    * This function is executed by the overlying MToken contract in response to a redeem action.
    * @param _reserve the address of the reserve
    ...
    * this is regulated by Mensa to ensure that the protocol is not abused, and the user is paying a fair
    ...
    * @param _receiveMToken true if the liquidators wants to receive the MTokens, false if
    ...
    **/
    function liquidationCall(
        ...
        liquidationManager = MensaLiquidationManager(addressesProvider.getMensaLiquidationManager());
